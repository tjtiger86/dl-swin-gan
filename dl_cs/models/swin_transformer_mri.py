...
class PatchUnembed3d(nn.Module):
    ...
    def forward(self, x: Tensor, pre_size: List[int]) -> Tensor:
        ...
        x = x.permute(0, 4, 1, 2, 3)  # B C T Wh Ww
        return x
...
class SwinTransformer3d(nn.Module):
    ...
    Args:
        ...
        in_channel: Number of channels, for most images, RBG = 3, but for MRI = (real, imag) * num_emaps, Default: None
    """
    ...
    def __init__(
        ...
        in_channel: Optional[Callable[..., nn.Module]] = None,
        ...
        patch_unembed = PatchUnembed3d
        

        # split image into non-overlapping patches
        self.patch_embed = patch_embed(patch_size=patch_size, embed_dim=embed_dim, norm_layer=norm_layer, in_channels=in_channel)
        self.patch_unembed = patch_unembed(patch_size=patch_size, embed_dim=embed_dim, norm_layer=norm_layer, in_channels=in_channel)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # print("Inside swin_transformer. Size of x after patch_embed is {}".format(x.size()))
        x = self.pos_drop(x)
        x = self.features(x)  # B _T _H _W C
        x = self.norm(x)
        #print("Inside swin_transformer. Size of x after fetures is {}".format(x.size()))
        x = x.permute(0, 4, 1, 2, 3)  # B, C, _T, _H, _W
        #print("Inside swin_transformer. Size of x after permute is {}".format(x.size()))
        x = self.patch_unembed(x, x_size)
        # print("Inside swin_transformer. Size of x after unembed is {}".format(x.size()))
...
def swin3d_t_mr(*, weights: Optional[Swin3D_T_Weights] = None, progress: bool = True, **kwargs: Any) -> SwinTransformer3d:
    """
