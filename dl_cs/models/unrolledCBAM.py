...
from dl_cs.models.CBAM import CBAMResNet
from dl_cs.mri.algorithms import ConjugateGradient


class UnrolledCBAMResNet(nn.Module):
    """
    Abstract class for an unrolled neural network.
    """
    def __init__(self, config):
        super(UnrolledCBAMResNet, self).__init__()
    ...
    def init_nets(self):
        ...
        CBAMresnet_params = dict(in_chans=in_chans,
        ...
        if self.share_weights:
            ...
            nets = nn.ModuleList([CBAMResNet(**CBAMresnet_params)] * self.num_unrolls)
        else:
            # Initializes unique networks
            nets = nn.ModuleList([CBAMResNet(**CBAMresnet_params) for _ in range(self.num_unrolls)])
...
class ProximalGradientDescent(UnrolledCBAMResNet):
    """
...
class HalfQuadraticSplitting(UnrolledCBAMResNet):
    """
