...
class GlobalMaxPool(nn.Module):
    """
    A generic class for Max pooling of layers
    """
    def __init__(self):
...
class CABlock(nn.Module):
    ...
    This is esentially the Squeeze excitation block proposed by Hu. et. al.
...
class SeResNet(nn.Module):
    ...
    def __init__(self, num_resblocks, in_chans, chans, kernel_size, rr, act_type='relu', use_complex_layers=False, circular_pad=True):
        ...
        for _ in range(num_resblocks):
            self.se_res_blocks += [SeResBlock(chans, kernel_size, rr, act_type=act_type, is_complex=use_complex_layers)]
