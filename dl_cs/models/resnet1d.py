"""
Implementations of various layers and networks for 3-D data [N, H, C].
...
class Normalization(nn.Module):
    ...
    def __init__(self, in_chans, type):
        ...
        if type == 'none':
            ...
            self.norm = nn.InstanceNorm1d(in_chans, affine=False)
        elif type == 'batch':
            self.norm = nn.BatchNorm1d(in_chans, affine=False)
        else:
...
class Conv1d(nn.Module):
    """
    A simple 1D convolutional operator.
    """
    def __init__(self, in_chans, out_chans, kernel_size):
        ...
        self.conv = nn.Conv1d(in_chans, out_chans, kernel_size, padding=padding)
...
class ComplexConv1d(nn.Module):
    """
    A 1D convolutional operator that supports complex values.
    ...
    def __init__(self, in_chans, out_chans, kernel_size):
        ...
        self.conv_r = nn.Conv1d(in_chans, out_chans, kernel_size, padding=padding)
        self.conv_i = nn.Conv1d(in_chans, out_chans, kernel_size, padding=padding)
...
class ConvBlock(nn.Module):
    """
    A 1D Convolutional Block that consists of Norm -> ReLU -> Dropout -> Conv
    ...
    def __init__(self, in_chans, out_chans, kernel_size,
        ...
        self.name = 'ComplexConv1D' if is_complex else 'Conv1D'
        ...
        if is_complex:
            convolution = ComplexConv1d(in_chans, out_chans, kernel_size=kernel_size)
        else:
            convolution = Conv1d(in_chans, out_chans, kernel_size=kernel_size)
...
class ResNet(nn.Module):
    """
    Prototype for 1D ResNet architecture.
    """
    ...
    def _postprocess(self, output):
        ...
        if not self.use_complex_layers:
            ...
            # [N, 2*C, T] float32 -> [N, C, T] complex64
    ...
    def forward(self, input):
        """
        Args:
            input (torch.Tensor): Input tensor of shape [N, H, C] and type torch.complex64

        Returns:
            (torch.Tensor): Output tensor of shape [N, H, C] and type torch.complex64
        """
