...
class PatchExpand(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        ...
        self.norm = norm_layer(dim // 2) #check this one... 

    def forward(self, x: Tensor, input_resolution: List[int]) -> Tensor:
        """ Forward function.
        Args:
            x: Input feature, tensor size (B, D, H, W, C).
        """

        _, _, _, H, W = input_resolution
        x = self.expand(x)

        B, _, _, _, C = x.shape

        x = rearrange(x, 'b d h w (p1 p2 c)-> b d (h p1) (w p2) c', p1=2, p2=2, c=C//4)
        x = center_crop(x, dims=[2,3], shapes=[H, W])
        x = self.norm(x)
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        # generate image from non-overlapping patches 
        self.patch_unembed = PatchUnembed3D(
            patch_size=patch_size, embed_dim=embed_dim, 
            norm_layer=norm_layer if self.patch_norm else None, in_channels=in_chans)
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                norm_layer=norm_layer,
                #We Don't have to downsample in swinIR 
        ...
        for i_layer in range(self.num_layers-1):
            layer_up = PatchExpand(dim=int(embed_dim * 2**(self.num_layers-i_layer-1)))
    ...
    def forward(self, x):
        ...
        for ii, layer in enumerate(self.layers):
            x_size.append(x.size()) if ii < self.num_layers -1 else None 
            x = layer(x.contiguous())
        
        #Combination of Upsampling + normalization
        x = rearrange(x, 'n c d h w -> n d h w c')

        for jj, layers_up in enumerate(self.layers_up):
            x = layers_up(x.contiguous(), x_size[self.num_layers-jj-1])
        
        #Going back up original dimension 
        x = rearrange(x, 'n d h w c -> n c d h w')
        x = self.patch_unembed(x, x_size[0])   
