...
class LitUnrolled(pl.LightningModule):
    ...
    def forward(self, kspace, maps, mask, L_init, R_init):
        if self.data_parallel:
            # Necessary because nn.DataParallel converts tensors to float32 (why..?)
            kspace = torch.view_as_complex(kspace)
            maps = torch.view_as_complex(maps)
            L_init = torch.view_as_complex(L_init)
            R_init = torch.view_as_complex(R_init)

        image_shape = (1, maps.shape[1]) + tuple(kspace.shape[2:])

        # Initialize linops
        A = T.SenseModel(maps, weights=mask)
        BlockOp = lr.ArrayToBlocks(self.block_size, image_shape, overlapping=self.overlapping).to(kspace.device)

        return self.model(y=kspace, A=A, BlockOp=BlockOp, L0=L_init.squeeze(0), R0=R_init.squeeze(0))
...
class DataTransform:
    ...
    def __call__(self, kspace, maps):
        ...
        decompose = lr.Decompose(self.block_size, self.num_basis, image_shape, overlapping=self.overlapping)
        ...
        return kspace, maps, mask, L_init, R_init, scale
...
def main(args):
    ...
    pbar = tqdm(total=len(data_loader), desc='DSLR Recon', unit='slice')
    ...
    for data in data_loader:
        # Load batch of data
        kspace, maps, mask, L_init, R_init, scale = data
        ...
        R_init = R_init.to(device)

        # Perform inference
        images = unrolled(kspace, maps, mask, L_init, R_init)
