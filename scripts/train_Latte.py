...
from dl_cs.models import unrolledLatte
...
class LitUnrolled(L.LightningModule):
    ...
    def __init__(self, config):
        ...
        if config.MODEL.META_ARCHITECTURE == 'dlespirit':
            self.model = unrolledLatte.ProximalGradientDescent(config)
        elif config.MODEL.META_ARCHITECTURE == 'modl':
            self.model = unrolledLatte.HalfQuadraticSplitting(config)
        elif config.MODEL.META_ARCHITECTURE == 'DDPM_X':
            self.model = unrolledLatte.DataConsistency(config)
        elif config.MODEL.META_ARCHITECTURE == 'DDPM_E':
            self.model = unrolledLatte.DDPM(config)
    ...
    def log_data(self, initial_guess, prediction, noise_im, target, mask, mask2, flag=0):
        ...
        if flag == 0:
            ...
            label = ["EMA Magnitude", 'EMA Phase', 'EMA Error', 'EMA Mask']
    ...
    def training_step(self, batch, batch_idx):
        ...
        if config.MODEL.META_ARCHITECTURE == 'DDPM_X':
            ...
            #loss_dict, pred, init_guess_noise = self.diffusion.training_losses(self.model, target, t, model_kwargs)
        ...
        if (self.global_step+1) % self.config.LOGGER.LOG_PREDICTION_EVERY_N_STEPS == 0:
            ...
            with torch.no_grad():
                gen_im = self.diffusion2.p_sample_loop_conditional(self.ema, initial_guess.shape, initial_guess, clip_denoised=False, 
                                                model_kwargs=model_kwargs, progress=True, device=self.device
                )
            self.log_data(initial_guess, gen_im, init_guess_noise, target, mask_p, mask_r, flag=2)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if config.MODEL.META_ARCHITECTURE == 'DDPM_X':
            ...
            #loss_dict, _, _ = self.diffusion.training_losses(self.model, target, t, model_kwargs)
