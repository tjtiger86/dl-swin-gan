"""
Training script for MRI unrolled recon.
"""
import random
import logging
import argparse
...
class LitUnrolled(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        if config.MODEL.META_ARCHITECTURE == 'dslr-pgd':
            self.model = dslr.AltMinPGD(config)
        elif config.MODEL.META_ARCHITECTURE == 'dslr-cg-v1':
            self.model = dslr.AltMinCGv1(config)
        elif config.MODEL.META_ARCHITECTURE == 'dslr-cg-v2':
            self.model = dslr.AltMinCGv1(config)
        elif config.MODEL.META_ARCHITECTURE == 'modslr-v1':
            self.model = dslr.AltMinMoDLv1(config)
        elif config.MODEL.META_ARCHITECTURE == 'modslr-v2':
            self.model = dslr.AltMinMoDLv2(config)
        else:
            raise ValueError('Meta architecture in config file not recognized!')

        # DSLR-specific parameters
    ...
    def compute_metrics(self, prediction, target, is_training=True):
        ...
        metrics[f'{tag}/complex_l1'] = metric.l1(target, prediction)
        metrics[f'{tag}/complex_l2'] = metric.l2(target, prediction)
        metrics[f'{tag}/complex_psnr'] = metric.psnr(target, prediction)
        ...
        metrics[f'{tag}/mag_l1'] = metric.l1(mTarget, mPrediction)
        metrics[f'{tag}/mag_l2'] = metric.l2(mTarget, mPrediction)
        metrics[f'{tag}/mag_psnr'] = metric.psnr(mTarget, mPrediction)

        return metrics

    def log_data(self, initial_guess, prediction, target, mask):
        # Helper function for logging scalars
        def save_scalar(scalar, tag):
            self.logger.experiment.add_scalar(tag, scalar, global_step=self.global_step)
        ...
        # Save step sizes (only used in MoDSLR)
        if (self.config.MODEL.META_ARCHITECTURE == 'modslr-v1') or (self.config.MODEL.META_ARCHITECTURE == 'modslr-v2'):
            save_scalar(self.model.lambda_l, 'Lambda/L')
            save_scalar(self.model.lambda_r, 'Lambda/R')

        return

    def training_step(self, batch, batch_idx):
        # Load batch of input-output pairs
        kspace, mask, maps, L_init, R_init, initial_guess, scale, target = batch

        # Create linear operators
        SenseModel = T.SenseModel(maps, weights=mask)
        BlockOp = lr.ArrayToBlocks(self.block_size, target.shape, overlapping=self.overlapping).cuda()

        # Perform forward pass through unrolled network
        pred = self.model(y=kspace, A=SenseModel, BlockOp=BlockOp, L0=L_init.squeeze(0), R0=R_init.squeeze(0))
    ...
    def train_dataloader(self):
        ...
        preprocess = CinePreprocess(self.config, lr_decom=True, use_seed=False)
    ...
    def val_dataloader(self):
        # Initialize transform function (performs pre-processing on each batch)
        preprocess = CinePreprocess(self.config, lr_decom=True, use_seed=True)
...
def main(config, devices, ckpt_file=None):
    ...
    logger = TensorBoardLogger(save_dir=config.OUTPUT_DIR, name="exp")
