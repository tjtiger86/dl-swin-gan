...
def fft3c(x, order='C', do_orthonorm=True):
    if order == 'C':
        if do_orthonorm:
            scale = np.sqrt(np.prod(x.shape[-3:]))
        else:
            scale = 1.0
        return fftnc(x, (-3, -2, -1)) / scale
    else:
        if do_orthonorm:
            scale = np.sqrt(np.prod(x.shape[:3]))
        else:
            scale = 1.0
        return fftnc(x, (0, 1, 2)) / scale


def ifft3c(x, order='C', do_orthonorm=True):
    if order == 'C':
        if do_orthonorm:
            scale = np.sqrt(np.prod(x.shape[-3:]))
        else:
            scale = 1.0
        return ifftnc(x, (-3, -2, -1)) * scale
    else:
        if do_orthonorm:
            scale = np.sqrt(np.prod(x.shape[:3]))
        else:
            scale = 1.0
        return ifftnc(x, (0, 1, 2)) * scale